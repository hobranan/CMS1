import { createUc01Uc02Runners } from "../../support/uc01_uc02_runner.js";
...
function formatError(error) {
  if (error && typeof error === "object" && "message" in error && error.message !== undefined && error.message !== null) {
...
function buildUcScenarioListFromSources(ucId, sources) {
  const explicit = [...(sources.scenarios[ucId] ?? []), ...(sources.autoplay[ucId] ?? [])];
  const atEntries = sources.atCatalog[ucId] ?? [];
...
function buildUcScenarioList(ucId) {
  return buildUcScenarioListFromSources(ucId, {
    scenarios: UC_SCENARIOS,
    autoplay: UC_AUTOPLAY_SCENARIOS,
    atCatalog: UC_AT_CATALOG
  });
}

const { runUc01Scenario, runUc02Scenario } = createUc01Uc02Runners({ callApi, payloadErrors, payloadCode, formatStep });
  ...
  for (const scenario of scenarios) {
    ...
    scenarioBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        setOutput(item.id, `Scenario failed: ${formatError(error)}`);
      } finally {
...
export const __appTestHooks = {
  ...
  runPlannedScenario,
  buildUcScenarioList,
  buildUcScenarioListFromSources,
  firstDefined,
  allTrue,
  formatError,
  makeUniqueEmail,
