...
test("gateway canceled confirmation keeps registration unpaid", () => {
  const system = createUc20System();
  const user = system.addUser("attendee20-cancel@example.test");
  const registrationId = "reg-20-3";
  ...
  const response = system.call("/api/v1/payments/gateway/confirm:POST", {
    body: {
      ...
      gatewayReference: "gw-ref-201",
      signature: "valid-signature"
    }
  });

  assert.equal(response.status, 200);
  assert.equal(response.body.outcome, "canceled");
