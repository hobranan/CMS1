...
export function createUc16System() {
  ...
  function addEditor(email, role = "editor") {
    const user = repository.createActiveUser({
      email,
      passwordHash: hashPassword("EditorPass!123"),
  ...
  return {
    deps,
    addEditor,
    seedConference(input) {
      deps.scheduleDraftRepository.seedConference(input);
    },
    failNextSave() {
      deps.scheduleDraftRepository.failNextDraftSave();
    },
